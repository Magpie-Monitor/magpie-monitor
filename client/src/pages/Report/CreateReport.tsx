...
const CreateReport = () => {
    ...
    const [generationPeriod, setGenerationPeriod] = useState<string>(
        schedulePeriodOptions.periods[2],
    );
    const navigate = useNavigate();
    const [startDateMs, setStartDateMs] = useState<number>(
        getStartOfDay(Date.now()),
    );
    const [endDateMs, setEndDateMs] = useState<number>(getEndOfDay(Date.now()));
    ...
    useEffect(() => {
        const fetchData = async () => {
            const {
                notificationChannels: fetchedNotificationChannels,
                applications: fetchedApplications,
                nodes: fetchedNodes,
            } = await fetchClusterData(id || '');
                    ...
                    <div className="on-demand-report__row">
                        <div className="on-demand-report__row">
                            ...
                            <ReportGenerationType
                                setParentGenerationType={setGenerationType}
                            />
                        </div>
