...
func NewAccuracyFilter[T repositories.Log](accuracy insights.Accuracy) *AccuracyFilter[T] {
...
var AccuracyKeywords = map[insights.Accuracy][]string{
	insights.Accuracy__Low:    LowAccuracyKeywords,
	insights.Accuracy__Medium: MediumAccuracyKeywords,
	insights.Accuracy__High:   HighAccuracyKeywords,
}
...
func CountFilterResultEntries[T any](logsByApplication map[string][]T) int {
	var logsCount = 0

	for _, logs := range logsByApplication {
		logsCount += len(logs)
	}
